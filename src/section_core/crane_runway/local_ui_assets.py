...
def render_local_ui_js() -> str:
    return """const LOCAL_UI_BETA_VERSION = 'V1-094';
...
function handleUiAction(action,event){ if(action==='load-template'){safeUiAction(()=>loadTemplate('ipn-with-cover'));} else if(action==='import-json-file'){safeUiAction(importJsonFile);} else if(action==='validate-case'){safeUiAction(validateCase);} else if(action==='run-case'){safeUiAction(runCase);} else if(action==='validate-and-run'){safeUiAction(validateAndRunCase);} else if(action==='apply-all-forms'){safeUiAction(applyAllFormsToJson);} else if(action==='refresh-visual-preview'){safeUiAction(refreshVisualPreview);} else if(action==='refresh-case-quality'){safeUiAction(refreshCaseQuality);} else if(action==='run-ui-diagnostics'){safeUiAction(runUiDiagnostics);} else if(action==='run-frontend-self-test'){safeUiAction(runFrontendSelfTest);} else if(action==='start-case-wizard'){safeUiAction(startCaseWizard);} else if(action==='previous-wizard-step'){safeUiAction(previousWizardStep);} else if(action==='next-wizard-step'){safeUiAction(nextWizardStep);} else if(action==='save-wizard-step'){safeUiAction(saveWizardStep);} else if(action==='reset-case-wizard'){safeUiAction(resetCaseWizard);} else if(action==='finish-case-wizard'){safeUiAction(finishCaseWizard);} else if(action==='load-wizard-template'){safeUiAction(()=>loadWizardTemplate());} else if(action==='open-advanced-json'){safeUiAction(openAdvancedJson);} else if(['start-setup','edit-case-setup','edit-loads','review-case','validate-case-from-workflow','run-case-from-workflow','view-results','export-outputs'].includes(action)){safeUiAction(()=>handleWorkflowCardAction(action));} else if(['new-demo-case','top-open-project','top-validate-run','top-view-results','top-export'].includes(action)){safeUiAction(()=>handleTopAction(action));} else if(action==='set-simple-mode'){safeUiAction(toggleSimpleMode);} else if(action==='set-expert-mode'){safeUiAction(toggleExpertMode);} else if(action&&action.startsWith('switch-tab-')){switchMainTab(action.replace('switch-tab-',''));} else {setAppShellStatus('Unknown UI action.');}}
...
const UI_MODE_STORAGE_KEY='craneRunway.uiMode';
function renderVisualWorkflowMap(){}
function updateVisualWorkflowMap(){setAppShellStatus('Visual workflow updated.');}
function getVisualWorkflowStatus(){return 'Pending';}
function setVisualWorkflowStatus(_step,_status){setAppShellStatus('Workflow action completed.');}
function handleWorkflowCardAction(action){setAppShellStatus('Workflow action started.'); if(action==='validate-case-from-workflow'){validateCase();} else if(action==='run-case-from-workflow'){runCase();} else if(action==='start-setup'){switchMainTab('project');} else if(action==='edit-case-setup'){switchMainTab('setup');} else if(action==='edit-loads'){switchMainTab('loads');} else if(action==='review-case'){switchMainTab('review');} else if(action==='view-results'){switchMainTab('results');} else if(action==='export-outputs'){switchMainTab('export');} else {setAppShellStatus('Workflow action needs attention.');}}
function setUiMode(mode){try{localStorage.setItem(UI_MODE_STORAGE_KEY,mode);}catch(_e){} setAppShellStatus(mode==='simple'?'Simple mode enabled.':'Expert mode enabled.');}
function getUiMode(){try{return localStorage.getItem(UI_MODE_STORAGE_KEY)||'simple';}catch(_e){return 'simple';}}
function applyUiMode(){}
function toggleSimpleMode(){setUiMode('simple');applyUiMode();}
function toggleExpertMode(){setUiMode('expert');applyUiMode();}
function handleTopAction(action){setAppShellStatus('Top action started.'); if(action==='new-demo-case'){loadTemplate('ipn-with-cover');switchMainTab('setup');} else if(action==='top-open-project'){switchMainTab('project');} else if(action==='top-validate-run'){validateAndRunCase();} else if(action==='top-view-results'){switchMainTab('results');} else if(action==='top-export'){switchMainTab('export');}}
function computeNextUserAction(){return 'Start by loading a demo template or creating/opening a project.';}
function renderNextUserAction(){}
function updateNextUserAction(){}

function initializeLocalUi(){setupActionHandlers();applyUiMode();renderVisualWorkflowMap();renderNextUserAction();}
